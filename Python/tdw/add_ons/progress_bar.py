...
class ProgressBar(UI):
    """
    Two rectangles used to display a value between 0 and 1.
    The value is public and can be set or incremented by a Trial.
    """
    ...
    def __init__(self, value: float = 0, increment: bool = True, left_to_right: bool = True, size: Dict[str, int] = None,
        ...
        super().__init__(canvas_id=canvas_id, can_use_existing=True)
        """:field
    ...
    def get_initialization_commands(self) -> List[dict]:
        ...
        self.add_image(image=self._get_image(self._underlay_color), position=self._position, size=self._size,
                       rgba=False, anchor=self._anchor, pivot=self._pivot, raycast_target=False)
        ...
        self._overlay_id = self.add_image(image=self._get_image(self._overlay_color), position=overlay_position,
                                          size=self._size, rgba=False, anchor=self._anchor, pivot=overlay_pivot,
    ...
    def _get_image(self, color: Tuple[int, int, int]) -> bytes:
        """
        ...
        with BytesIO() as output:
            Image.new(mode="RGB", size=(self._size["x"], self._size["y"]), color=color).save(output, "PNG")
