...
class Ninja(Trial):
    ...
    def _get_trial_initialization_commands(self) -> List[dict]:
        ...
        commands = [WebGLController.get_add_scene(scene_name=self._scene_names[np.random.randint(0, len(self._scene_names))]),
    ...
    def _add_object(self, commands: List[dict], name: str, positions: List[np.ndarray], object_id: int) -> int:
        ...
        # Add the object.
        commands.extend(WebGLController.get_add_physics_object(model_name=name,
                                                               object_id=object_id,
                                                               position=TDWUtils.array_to_vector3(position),
                                                               rotation=TDWUtils.array_to_vector3(
                                                                   np.random.uniform(-360, 360, 3)),
