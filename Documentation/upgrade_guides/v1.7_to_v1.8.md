...
#### Bug: Flex doesn't work in Linux

This seems to have been an issue since at least TDW v1.7. We're working on fixing this.

Additionally, there is a new problem in which Flex shaders cause Unity Editor to crash to desktop while creating the Linux build (see below). These will prevent the `"draw_particles"` in the Flex commands from working properly in Linux, and will likely be unsolvable until Unity fixes the underlying issue in an upcoming release.
