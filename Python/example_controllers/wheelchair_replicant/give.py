...
class Give(Controller):
    def run(self) -> None:
        ...
        replicant_state: _ReplicantState = _ReplicantState.waiting_for_magnebot
        # Add a Magnebot.
        magnebot: Magnebot = Magnebot(robot_id=magnebot_id,
                                      position={"x": -0.5, "y": 0, "z": 1.2},
                                      rotation={"x": 0, "y": 180, "z": 0})
        magnebot.collision_detection.objects = False
        magnebot_state: _MagnebotState = _MagnebotState.moving_to_cabinet
        ...
        # Initialize the Magnebot.
        while magnebot.action.status == MagnebotActionStatus.ongoing:
            self.communicate([])
        self.communicate([])
        ...
        while not done:
            ...
            if magnebot.action.status != MagnebotActionStatus.ongoing:
                ...
                if magnebot_state == _MagnebotState.moving_to_cabinet:
                    magnebot_state = _MagnebotState.reaching_for_target
                ...
                elif magnebot_state == _MagnebotState.reaching_for_target:
                    magnebot_state = _MagnebotState.grasping_target
                    magnebot.grasp(target=target_id, arm=MagnebotArm.right)
                # The Magnebot finished grasping the target object. Reset the arm.
                elif magnebot_state == _MagnebotState.grasping_target:
                    magnebot_state = _MagnebotState.resetting_arm_after_grasping
                    magnebot.reset_arm(arm=MagnebotArm.right, set_torso=True)
                # The Magnebot finished resetting its arm. Move to the Replicant.
                elif magnebot_state == _MagnebotState.resetting_arm_after_grasping:
                    magnebot_state = _MagnebotState.moving_to_replicant
                ...
                # The Magnebot finished moving to the Replicant. Reach for a position between the two agents.
                elif magnebot_state == _MagnebotState.moving_to_replicant:
                    # Get a midpoint.
                    ...
                    magnebot_state = _MagnebotState.reaching_for_replicant
                    ...
                    replicant_state = _ReplicantState.reaching_for_object
                # The Magnebot and Replicant finished reaching for the position. Drop the object.
                elif magnebot_state == _MagnebotState.reaching_for_replicant:
                    if replicant_state == _ReplicantState.reaching_for_object and replicant.action.status != ReplicantActionStatus.ongoing:
                        magnebot_state = _MagnebotState.dropping_object
                ...
                elif magnebot_state == _MagnebotState.dropping_object:
                    magnebot_state = _MagnebotState.resetting_arm_after_dropping
                    magnebot.reset_arm(arm=MagnebotArm.right)
                # The Magnebot finished resetting the arm. Move away from the Replicant.
                elif magnebot_state == _MagnebotState.resetting_arm_after_dropping:
                    magnebot_state = _MagnebotState.moving_away_from_replicant
                    magnebot.move_by(distance=-1)
                # We're done!
                else:
                    done = True
            # The Replicant finished an action.
            if replicant_state != _ReplicantState.waiting_for_magnebot and replicant.action.status != ReplicantActionStatus.ongoing:
                # The Replicant finished reaching for the object. Try to grasp the object.
                if replicant_state == _ReplicantState.reaching_for_object:
                    # The object can be grasped.
                    if target_id not in magnebot.dynamic.held[MagnebotArm.right]:
                        replicant_state = _ReplicantState.grasping_object
                        replicant.grasp(target=target_id, arm=ReplicantArm.right)
                # The Replicant finished grasping the object. Turn a little.
                elif replicant_state == _ReplicantState.grasping_object:
                    replicant_state = _ReplicantState.moving_away_from_magnebot
