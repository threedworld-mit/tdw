...
class ReachForAffordancePoint(Controller):
    ...
    def reach_for(self, resp: List[bytes], target: Union[int, np.ndarray, Dict[str,  float]], arm: Arm, hand_position: np.ndarray) -> List[dict]:
        ...
        return [
            ...
            # Reach for IK target
            {"$type": "humanoid_reach_for_position", "position": target, "id": humanoid, "length": reach_action_length, "arm": arm}
        ]
    ...
    def run(self):
        ...
        for i in range(1):
            ...
            if pos_val < 0:
                ...
                commands = self.reach_for(resp=resp, arm=Arm.left, target=self.t_id, hand_position=np.array([0, 1, 0]))
                self.communicate(commands) 
            else:
                #self.reach_for_target(arm="right", humanoid=self.h_id, target=self.t_id, reach_action_length=40, reset_action_length=60)
                commands = self.reach_for(resp=resp, arm=Arm.right, target=self.t_id, hand_position=np.array([0, 1, 0]))
                self.communicate(commands) 
