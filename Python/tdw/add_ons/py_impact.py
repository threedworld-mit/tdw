...
class PyImpact(CollisionManager):
    ...
    def get_scrape_sound(self, velocity: np.array, contact_normals: List[np.array], primary_id: int,
        ...
        # Initialize scrape variables; if this is an in-process scrape, these will be replaced by the stored values. 
        scrape_event_count = 0
        summed_master =np.zeros(SAMPLE_RATE*10) #Make a 10sec scrape audio file; Can be increased if the application needs a longer scrape
        ...
        if scrape_material not in self.scrape_surface_data:
            ...
            rough_ratio = (np.std(scrape_surface) / (3* 10 ** -4)) ** 0.85
        ...
        # The sound works best for video frame rates below 50 Hz
        frame_rate=50
        dist = (mag / frame_rate)
        ...
        delta_frame = int(SAMPLE_RATE/frame_rate)+1

        vect1 = np.linspace(0, 1, num_pts)
        vect2 = np.linspace(0, 1, delta_frame)
        ...
        summed_master[scrape_event_count*delta_frame:(scrape_event_count*delta_frame+len(conv_s))] = summed_master[scrape_event_count*delta_frame:(scrape_event_count*delta_frame+len(conv_s))] +conv_s
        ...
        self._scrape_events_count[scrape_key] = scrape_event_count
        # Converting the sound to Base64 for sending to unity
