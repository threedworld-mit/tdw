...
def struct_py(struct: Struct) -> None:
    ...
    constructor_fields = [PyField(field=f) for f in constructor_fields if f.cs_field_type != "TrialStatus"]
    for field in constructor_fields:
        constructor_parameter = f"{field.field.name}: {field.py_field_type}"
        param_docstring = f"{INDENT_8}:param {field.field.name}: {field.field.description}"
        if field.py_default_value is None:
            ...
            if field.field.is_collection or field.is_mutable_type:
    ...
    for field in struct.inherited_fields:
        if PyField(field=field).py_field_type == "TrialStatus":
            continue
        super_fields.append(f"{field.name}={field.name}")
    for field in [PyField(f) for f in struct.fields]:
        if field.field.constant or field.field.static:
            continue
        # This field needs to have a None default value and it needs to get set within the constructor.
        if (field.field.is_collection or field.is_mutable_type) and field.py_default_value is not None:
            field_declaration = (f"{INDENT_8}if {field.field.name} is None:\n{INDENT_12}" +
                                 '""":field\n' + INDENT_12 + field.field.description + "\n" + INDENT_12 + '"""\n' +
                                 f"{INDENT_12}self.{field.field.name}: {field.py_field_type} = { field.py_default_value}\n{INDENT_8}else:\n{INDENT_12}self.{field.field.name} = {field.field.name}")
        else:
            if field.py_field_type == "TrialStatus":
                field_value = "TrialStatus.initializing"
            else:
                field_value = field.field.name
            # This is the field declaration if the type is not a collection.
            field_declaration = INDENT_8 + '""":field\n' + INDENT_8 + field.field.description + "\n" + INDENT_8 + '"""\n' + INDENT_8 + f"self.{field.field.name}: {field.py_field_type} = {field_value}"
