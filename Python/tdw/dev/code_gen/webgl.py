...
def namespace_py(namespace: Namespace, enums: Dict[str, EnumType]) -> None:
    ...
    for enum in namespace.enums:
        if enum.name in EXCLUDED_TYPES:
            continue
        enum.get_py_doc()
        enum_py(enum)
    ...
    for klass in namespace.klasses:
        if klass.name in EXCLUDED_TYPES:
            continue
        klass.get_py_doc(enums)
        struct_py(klass)
    write_init("trials")
    write_init("trial_adders")
...
def write_py_docs(assembly: Assembly) -> None:
    ...
    root_directory = CONFIG.tdw_docs_path.joinpath("docs/webgl/py/api").resolve()
...
def write_cs_docs(assembly: Assembly) -> None:
    ...
    # Get the root output directory.
    root_directory = CONFIG.tdw_docs_path.joinpath("docs/webgl/cs/api").resolve()
    recreate_directory(root_directory)
    ...
    # Write the trials.
    write_cs_doc_directory(assembly=assembly,
                           namespace="TDW::WebGL::Trials",
                           root_directory=root_directory,
                           folder="trials")
    # Write the add-ons.
...
def write_init(folder: str) -> None:
    ...
    root_directory = CONFIG.tdw_path.joinpath(f"Python/tdw/webgl/{folder}").resolve()
    inits = []
