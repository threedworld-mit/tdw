...
The Oculus Touch rig can send basic haptics data. The rig has a small collider at its base. Each palm has a Rigidbody and a collider. The base and the palms will be detected if [collision detection is enabled](../physx/collisions.md) as if they were standard TDW objects. If you are using [Clatter](../clatter/overview.md) (see below), tapping your hands together will create a faint sound.
...
This example controller adds an Oculus Touch rig and [Clatter](../clatter/overview.md) to a scene:
...
- [oculus_touch_clatter.py](https://github.com/threedworld-mit/tdw/blob/master/Python/example_controllers/vr/oculus_touch_clatter.py) Listen to audio generated by Clatter.
