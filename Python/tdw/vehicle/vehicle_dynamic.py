...
class VehicleDynamic:
    ...
    def __init__(self, resp: List[bytes], vehicle_id: int, frame_count: int):
        """
        :param resp: The response from the build, which we assume contains `vehicle` output data.
        ...
        """:field
        The [camera projection matrix](../../api/output_data.md#cameramatrices) of the vehicle's camera as a numpy array.
        """
        self.projection_matrix: Optional[np.ndarray] = None
        """:field
        The [camera matrix](../../api/output_data.md#cameramatrices) of the vehicle's camera as a numpy array.
        """
        ...
        self.avatar_id = str(vehicle_id)
        for i in range(len(resp) - 1):
            r_id = OutputData.get_data_type_id(resp[i])
            # Get vehicle's transform data.
            if r_id == "tran":
                transforms = Transforms(resp[i])
                for j in range(transforms.get_num()):
                    if transforms.get_id(j) == vehicle_id:
            ...
            # Get the images captured by the avatar's camera.
                ...
                # Get this vehicles's avatar and save the images.
