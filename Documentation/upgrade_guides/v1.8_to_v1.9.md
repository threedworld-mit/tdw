...
All non-API documentation has been completely rewritten. Even if you are an experienced TDW user, we recommend you read it; you might find something that makes your code easier to use or more efficient.
...
[`PyImpact` is now an add-on](../lessons/audio/py_impact.md) and now has scrape sounds. The code for generating audio sounds is mostly the same but the code for adding PyImpact to a controller is very differently, and a lot easier to use.

This is the code to required a physics impact audio event in v1.8:
