...
**SIGNIFICANT** speed-up in jpg/png encoding, thereby speeding up nearly every application of TDW by 33-75%. This is the most significant speed up in TDW since 2019 (prior to the initial public release).
...
| `_img` png                                  | 33%      |
| `_img` jpg                                  |          |
| `_img, _id, _depth` (all png)               | 48%*     |
| All image passes at the same time (all png) | 40%      |
| `_depth` or `_depth_simple`                 | 0%**     |
...
\*\* `_depth` and `_depth_simple` passes haven't changed because they return raw RGB data, not an encoded image.
